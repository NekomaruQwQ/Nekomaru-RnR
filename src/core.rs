...
pub fn is_active(hwnd: HWND) -> bool {
    // Exclude windows that are not visible, minimized and maximized. We do not
    // want to include those in the list of windows that can be centered, resized
...
pub fn is_centered(hwnd: HWND) -> Result<bool> {
    get_monitor_info_from_window(hwnd)
        .map_or(Err(Error::empty()), |monitor_info| {
            ...
            Ok(window_center == screen_center)
        })
}
